...
describe('autoEligiblityConfig Config Test', () => {
  ...
  test('Invalid env var throws error', () => {
    try {
      ...
      expect(err.message).toBe('The auto eligibility configuration config is invalid. "emailTemplateIds.waitingList" must be a valid GUID. "emailTemplateIds.applyServiceInvite" must be a valid GUID')
    }
  })
})
